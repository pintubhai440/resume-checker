    ...
    if not resume_text.strip() or not job_description.strip(): 
        ...
        with st.spinner('🔍 Gemini is performing comprehensive analysis... This might take 20-30 seconds.'): 
            llm = ChatGoogleGenerativeAI( 
                model="2.0-flash-thinking-exp-1219", 
            ...
            try: 
                ...
                **PRE-CALCULATED SKILL ANALYSIS (Use this as ground truth):** - Matched Skills: {matched_skills} 
                - Missing Critical Skills: {missing_skills} 
                - Skill Match Percentage: {skills_match_percentage}% 

                **YOUR TASK:** Based on all the provided information, complete the following JSON structure. 
                ...
                **RETURN ONLY RAW JSON IN THIS EXACT FORMAT:** {{ 
                ...
                .matched-skill {
                    ...
                    color: #0D6938; /* Dark green */
                    border: 1px solid #A3D4B6;
                }
                .missing-skill {
                    background-color: #FFF3D4; /* Light orange/yellow */
                    color: #B47D00; /* Dark orange/yellow */
                ...
                with skill_col1:
                    ...
                    if analysis_result['matched_skills']:
                        # Join all skill badges into a single HTML string
                        skills_html = "".join([f'<span class="skill-badge matched-skill">{skill}</span>' for skill in analysis_result['matched_skills']])
                ...
                with skill_col2:
                    ...
                    if analysis_result['missing_skills']:
                        # Join all skill badges into a single HTML string
                        skills_html = "".join([f'<span class="skill-badge missing-skill">{skill}</span>' for skill in analysis_result['missing_skills']])
                        st.markdown(f"<div style='line-height: 2.0;'>{skills_html}</div>", unsafe_allow_html=True)
                    else:
